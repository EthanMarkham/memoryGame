...
class Game extends React.Component {
  ...
  componentDidMount(){
    fetch("http://localhost:3001/api/v1/gameBoard").then(response=> response.json()).then((data) => {
      let defaultSquares = Array(data.length).fill({value:"*", image:"http://localhost:3001/cards/back.PNG"})
      console.log(data);
      this.setState({
        answerSquares: data,
